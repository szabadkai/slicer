...
function rotateHull(hull: Point2D[], angle: number): Point2D[] {
...
function hullAABB(hull: Point2D[]): AABB {
  let minX = Infinity,
    minZ = Infinity,
    maxX = -Infinity,
    maxZ = -Infinity;
...
function hullArea(hull: Point2D[]): number {
  // Shoelace formula
  let area = 0;
  const n = hull.length;
  for (let i = 0; i < n; i++) {
...
const DEFAULT_ROTATION_ANGLES = Array.from({ length: 12 }, (_, i) => (i * Math.PI) / 12);
...
function hullsTooClose(a: Point2D[], b: Point2D[], gap: number): boolean {
  const axes = getAxes(a).concat(getAxes(b));
  for (const axis of axes) {
    const [minA, maxA] = projectHull(a, axis);
    const [minB, maxB] = projectHull(b, axis);
    // If projections are separated by >= gap on any axis → not too close
    if (maxA + gap <= minB || maxB + gap <= minA) return false;
  }
  return true; // overlapping or within gap on all axes
}

function translateHull(hull: Point2D[], dx: number, dz: number): Point2D[] {
