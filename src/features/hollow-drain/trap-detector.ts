...
export function detectTraps(
  ...
  for (const hole of drainHoles) {
    ...
    if (xi >= 0 && xi < nx && yi >= 0 && yi < ny && zi >= 0 && zi < nz) {
      ...
      if (grid[i] === INTERIOR) { grid[i] = REACHABLE; queue.push(i); }
    }
  }
  ...
  while (queue.length > 0) {
    ...
    const xi = cur % nx;
    for (let d = 0; d < 6; d++) {
      const nx2 = xi + dx[d], ny2 = yi + dy[d], nz2 = zi + dz[d];
      if (nx2 < 0 || nx2 >= nx || ny2 < 0 || ny2 >= ny || nz2 < 0 || nz2 >= nz) continue;
      const ni = idx(nx2, ny2, nz2);
      if (grid[ni] === INTERIOR) { grid[ni] = REACHABLE; queue.push(ni); }
    }
  }
  onProgress(0.8);
  ...
  for (let i = 0; i < grid.length; i++) {
    ...
    while (q2.length > 0) {
      ...
      for (let d = 0; d < 6; d++) {
        ...
        if (nx2 < 0 || nx2 >= nx || ny2 < 0 || ny2 >= ny || nz2 < 0 || nz2 >= nz) continue;
        const ni = idx(nx2, ny2, nz2);
        if (grid[ni] === INTERIOR && !visited[ni]) { visited[ni] = 1; q2.push(ni); }
      }
    }
    ...
    for (const ci of component) {
      ...
      sumX += center.x; sumY += center.y; sumZ += center.z;
      if (center.y > maxY) { maxY = center.y; maxI = ci; }
    }
