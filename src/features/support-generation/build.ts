...
export function sampleContactPoints(
  ...
  const density = opts.density === 'auto' ? computeAutoDensity(totalArea) : opts.density;
  ...
  for (let i = 0; i < overhangTriangles.length; i++) {
    ...
    for (let p = 0; p < numPoints; p++) {
      ...
      contacts.push({ ...point, triangleIndex: tri });
    }
  }
...
export function generatePillars(
  ...
  void opts; // params used for future routing complexity
