...
- **Collision mesh must use `DoubleSide`.** The temp mesh used for raycasting in `supports.ts` and `manual-pillar.ts` MUST use `THREE.MeshBasicMaterial({ side: THREE.DoubleSide })`. Without it, rays shot from inside the model (e.g., the tip-bottom point below an overhang) won't detect faces because backfaces are culled. This was the root cause of the pierce-through bug.
